# SecondBrain — Architecture Reference (v3.0)
...
2. **Capture first, organize later.** A concept with no collection beats a note not taken.
...
`collection` is a plain string field on `concepts`, not a foreign key — no setup step,
...
Tables: `concepts`, `tags`, `concept_tags` (M:N), `relations` (typed edges),
`pending_links` (unresolved wikilinks), `concepts_fts` (FTS5), `_meta`.
...
`concepts_fts` is an **external-content** FTS5 table (`content=concepts`). This has three
sharp edges that v2 got wrong:

- **Updates/deletes need the `'delete'` command, not raw DELETE.** On external-content
  tables a plain `DELETE FROM concepts_fts` corrupts the index. The triggers use
  `INSERT INTO concepts_fts(concepts_fts, rowid, title, content) VALUES('delete', ...)`.
- **A missing AFTER DELETE trigger orphans the index.** v2 had none, so hard-deletes left
  searchable ghosts. v2.1 adds `concepts_ad`.
- **FTS knows nothing about `deleted_at`.** A raw `MATCH` returns soft-deleted rows. Every
  search joins FTS `rowid → concepts.rowid` and filters `WHERE deleted_at IS NULL`. This is
  done in `SecondBrain.search`; do not query `concepts_fts` directly without that join.

Because `concepts.id` is a TEXT UUID, the FTS link is the implicit integer `rowid`, not `id`.
Joins go `concepts_fts.rowid = concepts.rowid`, then read `concepts.id` for output.

## 4. Wikilink resolution

- `[[Title]]` resolves against `concepts.title` case-insensitively, exact match first,
  most-recently-updated on ties (`idx_concepts_title_nocase`).
- **Resolution is frozen at write time.** When content is saved, each `[[link]]` resolves
  to a concrete target id stored in the relation row. Editing some *other* concept later
...
- Unresolved links go to `pending_links` (its own indexed table, not JSON metadata — so
  ...
  concept with a matching title is later created or restored, `_resolve_pending_to` promotes
  those pending rows to real `wikilink` relations.
- Disambiguate with qualifiers: `[[Transformers (NLP)]]` vs `[[Transformers (electrical)]]`.

## 5. Soft delete

`delete` sets `deleted_at`; the concept disappears from search, list, related, traverse,
collections, and stats-counts, but `restore` brings it back and re-derives its wikilinks.
`delete --hard` removes the row; `ON DELETE CASCADE` cleans `relations`, `concept_tags`,
and `pending_links`, and the `concepts_ad` trigger cleans FTS.
...
add(title, content, collection=None, tags=None, sources=None) -> Concept
get(id) -> Concept | None
update(id, title=None, content=None, tags=None, collection=None) -> Concept
delete(id, hard=False) -> bool
restore(id) -> bool
list(collection=None, tag=None, limit=20, offset=0) -> list[Concept]
...
`embeddings.concept_id` is the one improvement over the v2 contract (v2 omitted it, orphaning
vectors on hard delete).

## 7. v1 → v2 migration

```
memories.*  → concepts.*            (all fields preserved)
wing.name   → concept.collection    (string copy, FK dropped)
room name   → tag                  ("AI-Projects" room → "AI-Projects" tag)
memory_tags → concept_tags
...
| Operation | Target |
|---|---|
| FTS search (50K concepts) | < 100 ms |
...
- All read paths filter soft-deleted rows (search no longer leaks deleted concepts).
...
- `embeddings.concept_id` gains `ON DELETE CASCADE`.
