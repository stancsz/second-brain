...
| G32 | reliability | Window coherence: enforce/flag `valid_from <= valid_to` (a backwards window is currently storable) | Logged iter-19 handoff: G26 validates each date's FORMAT but not the relationship between them; `add(sb_valid_from="2025-01-01", sb_valid_to="2020-01-01")` persists a window that can never contain any as_of | 2 | 1 | 4 | 8.0 |
